...
在ROS中，`ros::spin()` 和 `ros::spinOnce()` 都是用于处理ROS消息的函数，但它们的运行方式和作用有所不同：

`ros::spin()`
- `ros::spin()` 是一个阻塞式函数，它会一直运行直到节点被关闭。
- 当调用 `ros::spin()` 后，ROS节点会进入一个无限循环，等待接收并处理来自ROS系统的消息。这个函数会一直阻塞当前线程，直到节点被关闭。
- 通常用于在节点中持续监听和处理ROS消息，例如订阅话题，执行回调函数等。

`ros::spinOnce()`
- `ros::spinOnce()` 是一个非阻塞式函数，它允许程序员手动控制ROS消息的处理。
- 当调用 `ros::spinOnce()` 时，它会处理一次ROS消息队列中的所有消息，并立即返回。如果没有待处理的消息，它会立即返回而不阻塞当前线程。
- 通常用于在自定义的循环中手动处理ROS消息，例如在某些特定条件下处理消息，或者在特定时间间隔内执行消息处理。

- `ros::spin()` 是一个持续运行、阻塞式的函数，用于保持节点处于活动状态，持续处理ROS消息。
- `ros::spinOnce()` 是一个非阻塞式函数，允许手动控制ROS消息的处理，用于在特定时机或循环中处理消息。

选择使用哪个函数取决于你的节点需求，如果需要节点持续运行并处理消息，则使用 `ros::spin()`；如果需要在特定条件下或自定义循环中处理消息，则使用 `ros::spinOnce()`。
